...
{
    if (ADC_theACLK_CLKEN & ADC_theACLK_CLKEN_MASK)
    {
...
*
*******************************************************************************/
void ADC_theACLK_SetModeRegister(uint8 clkMode) 
{
    ADC_theACLK_MOD_SRC |= clkMode & ADC_theACLK_MODE_MASK;
}
...
*
*******************************************************************************/
void ADC_theACLK_ClearModeRegister(uint8 clkMode) 
{
    ADC_theACLK_MOD_SRC &= ~clkMode | ~ADC_theACLK_MODE_MASK;
}
...
{
    return ADC_theACLK_MOD_SRC & ADC_theACLK_MODE_MASK;
}
...
void ADC_theACLK_SetSourceRegister(uint8 clkSource) 
{
    ...
    if (oldSrc != CYCLK_SRC_SEL_CLK_SYNC_D && clkSource == CYCLK_SRC_SEL_CLK_SYNC_D && currDiv == 0)
    {
        ...
        ADC_theACLK_MOD_SRC |= CYCLK_SSS;
        ADC_theACLK_MOD_SRC =
            (ADC_theACLK_MOD_SRC & ~ADC_theACLK_SRC_SEL_MSK) | clkSource;
    }
